// Forecast insight via Lovable AI gateway (Gemini 2.5 Flash).
//
// Standalone, dependency-free (pure fetch — same shape as generate-meal-plan,
// which deploys reliably; the esm.sh supabase-js import was failing at boot).
// The CLIENT computes every number and sends a `metrics` snapshot; this function
// only narrates and caches one row per period in `ai_insights` via PostgREST.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const cacheErr = await cacheInsight("forecast", metrics.periodLabel, metrics.periodStart, payload, MODEL);
